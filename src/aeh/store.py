...
class IncompleteMigrationChainError(StoreError):
    ...
    **later, far from the open**, as `sqlite3.OperationalError: no such column:
    ...
    aeh.orch, aeh.pkg, aeh.synth, aeh.agg, aeh.grade`
    ...
    `aeh.orch` again for #62's `orch_report_indexes`, `aeh.agg` for #92's
    `agg_confidence_columns`, and `aeh.grade` for the last, #101's
    `grade_submission_grade_key`).
...
#: contributor import lists (it owns Cohort's last migration now). #101's
#: `grade_submission_grade_key` moved it 16→17, and `aeh.grade` joined the contributor
#: import lists the same way.)
