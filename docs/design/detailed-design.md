...
| ID | Category | Requirement |
|---|---|---|
| NFR-INTEG-01 | Performance efficiency | Span verification shall be O(total span bytes) with no model call, and shall add under 1% to run wall clock, **measured on a full-scale run** (PERF-06's F-SYNTH drive, ~8k `verify` calls), excluding test-double reads. The scale is part of the clause: the share's denominator is the whole run, so a small-cohort drive reads several times higher for the same per-call cost (2026-09-23). |
...
| CT-INTEG-12 | perf | Span verification is O(total span bytes) with no model call and adds under 1% to run wall clock at PERF-06's scale (NFR-INTEG-01). It runs on **every** unit, which is only affordable because of that bound; a change making it model-assisted would change the run's cost structure. | `M-ORCH` |
