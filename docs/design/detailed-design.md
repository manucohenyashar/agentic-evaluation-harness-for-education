...
**Version:** 1.5  **Date:** 2026-09-02  **Status:** Draft
...
| 1.5 | 2026-09-02 | **Named the seams and the shapes the test plan already depends on**, from seven findings raised against `test-plan.md` v1.2 in PR #161. Added `FR-PROV-15`: the live implementations take `transport`, `clock`, `retention_answers` and `on_dispatch` as constructor arguments defaulting to the real ones, behaviour-neutral — the seam `TC-PROV-10`'s own *“Rung 0 — stub transport”* and §4.2's injected-clock row assumed and §3.2 never wrote down, and without which `rate_limit_wait_s` costs 40 s of real sleeping to assert. Specified three types the interfaces named and never defined: `RunCounters` (§3.2), `RetentionReport` (§3.2) and `WorkUnit` (§3.7, with the reason `student_name` is on it). Pinned `FR-PROV-12`'s counting semantics — a 429-provoked retry increments `transport_retries` **and** `rate_limited_calls`, the two are not a partition — because the module's own `rate_limited_calls`-as-a-share-of-dispatch alert is not computable under the partitioning reading. **One clause reworded**, not added: `CT-PROV-11` now names the `counters()` accessor it already implied; the clause count stands at 330. Additive under §4.7: every change writes down something a consumer already relied on | manual, from `test-plan.md` PR findings |
...
| FR-PROV-12 | The module shall count `transport_retries`, `rate_limited_calls`, `rate_limit_wait_s`, `tokens_in`, `tokens_out` and `cache_hit_rate`, and shall expose them as a `RunCounters` value through a `counters()` accessor on the provider, for persistence into `run_metrics` by `M-ORCH`. The six are **not a partition**: every attempt beyond the first increments `transport_retries` whatever provoked it — HTTP 429 included, since `FR-PROV-06` classifies it as a transport-class failure — while `rate_limited_calls` counts *calls throttled at least once*, which is what makes the “rate-limited share of dispatch” alert below computable. `cache_hit_rate` is token-weighted (`cached_prefix_tokens / tokens_in` over the run): a rate in `[0,1]`, never a count. | §8.5, §9.7 | 1 |
...
| FR-PROV-15 | Each live implementation shall take its environment-facing dependencies as constructor arguments defaulting to the real ones — the HTTP `transport`, the `clock` that backoff and `Retry-After` waits are measured against, and the `retention_answers` source `verify_retention` reads — and shall behave identically whether they are supplied or defaulted. It shall additionally accept an optional `on_dispatch` observer, a no-op by default, invoked once per dispatched model call. This is the deterministic-transport seam of the four-seam rule: without it `FR-PROV-06`'s retry classification, `FR-PROV-07`'s `Retry-After` handling, `FR-PROV-12`'s `rate_limit_wait_s` and `CT-PROV-13`'s “before the first dispatch” are stated but unassertable, because programming a 429 or an ambiguous retention answer means reaching inside the very code that does the counting. | §8.7, §12 | 1 |
...
class InferenceProvider(Protocol):
    ...
    def counters(self) -> RunCounters: ...       # FR-PROV-12; read by M-ORCH (CT-PROV-11)
...
@dataclass(frozen=True)
class RunCounters:               # FR-PROV-12. The names are contract (CT-PROV-14).
    transport_retries: int       # attempts beyond the first, whatever provoked them
    rate_limited_calls: int      # calls throttled at least once -- not a partition of the above
    rate_limit_wait_s: float     # total time spent honouring Retry-After and backoff
    tokens_in: int
    tokens_out: int
    cache_hit_rate: float        # cached_prefix_tokens / tokens_in over the run, in [0,1]

@dataclass(frozen=True)
class RetentionReport:           # FR-PROV-14. Says *which* models, not merely whether.
    confirmed: tuple[ModelRef, ...]      # zero-retention routing positively confirmed
    unconfirmed: tuple[ModelRef, ...]    # denied, absent, or ambiguous -- all three fail closed
    all_confirmed: bool                  # unconfirmed == (); the gate M-CONF and M-ORCH read
    evidence: Mapping[str, str]          # build_id -> what the provider answered, for the audit
```

The seams `FR-PROV-15` requires, stated as construction rather than left to be discovered:

```python
class Transport(Protocol):
    # One HTTP attempt, no retry logic: retry classification is FR-PROV-06's, above.
    def send(self, request: TransportRequest) -> TransportResponse: ...

@dataclass(frozen=True)
class TransportResponse:
    status: int
    headers: Mapping[str, str]   # Retry-After is read from here (FR-PROV-07)
    body: bytes

# Defaults are the real ones and FR-PROV-15 forbids any behaviour difference by injection.
LocalServerProvider(transport=..., clock=..., on_dispatch=...)
OpenRouterProvider(transport=..., clock=..., retention_answers=..., on_dispatch=...)
RecordedFixtureProvider(fixture_dir=..., clock=..., on_dispatch=...)   # never takes a transport
```
...
`RecordedFixtureProvider` takes no `transport` deliberately — `CT-PROV-10` makes it hermetic, and
a transport argument on it would be a socket the fast tier could be handed by accident. Doubling
the *transport* rather than the *provider* is what keeps `FR-PROV-12`'s counters under test: a stub
provider replaces the very code that does the counting, so it can only be used by consumers of
`M-PROV`, never to test `M-PROV` itself.
...
| CT-PROV-11 | state | Writes nothing directly. Its counters (`transport_retries`, `rate_limited_calls`, `rate_limit_wait_s`, `tokens_in`, `tokens_out`, `cache_hit_rate`) accumulate in memory as a `RunCounters` value, readable at any time through `counters()` on the provider, and are read by `M-ORCH`, which persists them to `run_metrics`. | `M-ORCH` |
...
`M-CONF` (NFR-PROV-05) — if it does, the seam has leaked. The `FR-PROV-15` constructor seams are *additive and behaviour-neutral*: supplying a transport, a clock, a retention source or a dispatch observer must not change a single outcome above, so `CT-PROV-02` substitutability is asserted with them supplied and with them defaulted. `RecordedFixtureProvider` is the canonical
...
- `TBD`: the wire shape of a zero-retention confirmation. `FR-PROV-15`'s `retention_answers` seam
  exists precisely because that shape is unknown: it abstracts *what the provider said about this
  build*, so `FR-PROV-14`'s fail-closed rule is testable before the shape is settled and stays
  testable if it changes.
...
@dataclass(frozen=True)
class WorkUnit:
    work_id: str                 # deterministic, FR-ORCH-02
    run_id: RunId
    stage: Stage
    student_ref: str
    student_name: str            # the ledger knows whose paper it is -- see the note below
    submission_id: str
    criterion_id: str
    submission_text: str
    judge: ModelRef | None       # None for a deterministic unit
    attempt: int
```

`student_name` on the unit is deliberate, and it is the reason `NFR-JUDGE-04`/`NFR-PROV-04` are
testable at all. The pseudonymization boundary is at **assembly**, not at the ledger: a run has to
know whose paper it is to return a grade, and `M-JUDGE`'s job (`FR-JUDGE-01`'s whitelist schema) is
to put `student_ref` in the request and leave the name behind. A `WorkUnit` carrying only
`student_ref` would hand the assembler nothing to leak, so `TC-PROV-21`, `SEC-04` and
`TC-PROV-C13` — which names *"a sentinel student name present in the fixture"* — would pass
against an assembler that copies every field it is given.
