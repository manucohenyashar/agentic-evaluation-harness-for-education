...
**Version:** 1.3  **Date:** 2026-08-30  **Status:** Draft
...
| ID | Kind | Clause (assertable) | Consumers |
|---|---|---|---|
| CT-STORE-01 | surface | `Store` yields exactly three handle kinds — `package(id)` (Tier P), `cohort(id)` (Tiers C+R, one file), `durable()` (Tier D) — plus `blobs()` and `purge_cohort(id)`. A `TierHandle` offers `query`, `enqueue_write`, and `transaction`, and nothing else. | every module except `M-CONF` |
...
| `M-STORE` | `CT-STORE` | 1.0 | stable | 18 | every module except `M-CONF`, `M-SETUP`, `M-CALIB`, `M-CONFORM` (which reach it through `M-PKG` or `M-ORCH`) |
