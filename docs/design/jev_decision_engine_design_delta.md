...
|---|---|---|---|
| FR-PROV-16 | The module shall expose a `DecisionProvider` protocol: `decide(request: DecisionRequest, model_ref: ModelRef) -> Decision`, `decision_capabilities(model_ref) -> DecisionCapabilities` *(named so in #440: `RecordedFixtureProvider` serves both surfaces and its `capabilities` already returns the LLM `Capabilities`; decision-only providers also answer to `capabilities`)*, `estimate_cost(plan: CallPlan) -> CostEstimate`, `verify_retention(model_refs) -> RetentionReport`. `decide` is synchronous and blocking, and one call is one HTTP request. | User directive; R27 | 1 |
...
| FR-CONF-20 | The decision `ModelRef` shall satisfy `is_resolved()` by the existing rules (CT-CONF-03). For `edge-local`, a weights path plus quantization plus `@sha256:` digest (e.g. `/models/openjev-FP8/model.safetensors@sha256:…`, quantization `fp8`, or the MLX 4-bit build with `q4`). *(Corrected in #444: FR-CONF-03's resolved form needs a weights-file suffix, so a bare directory path is unresolved.)* Otherwise a provider-pinned slug (e.g. `openrouter/typesafe/jev-1.13@2026-09-01`). A floating tag (`~typesafe/jev-latest`, `:latest`) is refused. The build is read from `HARNESS_JEV_BUILD`. | FR-CONF-03 | 1 |
...
| FR-JUDGE-25 | *(As shipped in #447: the numeral scan covers the Jev question strings, whose band levels are the declared descriptors, and the static directive at rubric strictness, plus the exemplar material at content strictness. The criterion and question fields carry the question's own prompt text and reference solution, which the LLM path renders unscanned, and the fenced submission is the student's own work. Scanning either would make the engine refuse ordinary maths questions or hand a student a lever over their own unit, so neither is scanned; this matches TC-JUDGE-32 (c).)* **Isolation and numeral scan carry over.** `assert_isolated` shall run on the `ScoringRequest` before derivation, as today. The FR-JUDGE-03 numeral scan shall run over the rendered `DecisionRequest`: `state`'s rubric-surface fields at rubric strictness and `submission` at content strictness, exactly as for `prompt_fields`, plus every question's `instructions`, `levels`, `when_true` and `when_false` at rubric strictness. A violation raises `IsolationViolation` and nothing is dispatched. | FR-JUDGE-01/02/03; CT-JUDGE-03 (safety property) | 1 |
...
| ID | Requirement | Traces to | Phase |
|---|---|---|---|
| FR-ORCH-36 | *(As shipped in #446: numbers use one canonical spelling, trailing zeros stripped, so the threshold is recorded as `"0.8"`; `0.8` and `0.80` are one work identity, per FR-CONF-22. The OOM panel-drop rewrite keeps this key.)* `panel_config_json(panel, *, decision_engine=None)` shall add a key `"decision_engine": {"build": …, "provider": …, "threshold": "0.80", "cite_threshold": "0.50", "max_citation_questions": 16, "token_bytes_ratio": 3, "template": "judge-decision/1"}` **only when** an engine is present. Without an engine, the string is byte-identical to today. `panel_config` is already a `work_id` input (FR-ORCH-01), so turning the engine on, changing its build or changing its threshold yields new work ids and never mixes engines under one id. | FR-ORCH-01; CT-CONF-14 | 1 |
...
| FR-PROV-33 | **Shim placement.** The shim shall live at `tools/openjev_small_shim/` with its own `requirements.txt` (torch, transformers ≥ 5.17, pinned). It vendors `openjev_decide.py` and `modeling_openjev.py` byte-exact at a pinned Hugging Face revision, with their sha256 values in the shim's `VENDOR.md`. *(As shipped in #441: the shim imports only `modeling_openjev.py`. It reimplements `openjev_decide.py`'s template and single-window scoring rather than calling `decide()`, which windows long states and imports torch at top level; `openjev_decide.py` is kept for verification.)* No module under `src/aeh` imports it, and it imports nothing from `aeh`. `requirements-dev.txt` and `pyproject.toml` do not gain torch. | CT-PROV-15; FR-PROV-03 | 1 |
| FR-PROV-34 | **Shim translation.** *(As shipped in #441: `translate(document) -> (state, [Translated(key, type, options, hypotheses)])`, `answer(translated, entailment) -> answer` and `handle(document, scorer) -> (status, body)`. Rubric texts are embedded straight into the vendor hypothesis template rather than carried through `RUBRIC_MARK`, which gives the same hypotheses.)* The shim's translation and answering shall be pure functions, testable without torch through an injected scorer: <br>• **Noul:** options `["no","yes"]`; `criteria.false`/`true` become their rubric texts via the vendor `RUBRIC_MARK` convention; the answer is `{type:"noul", noul: p_yes}`. <br>• **Choice:** options are the criteria keys, with descriptions as rubric texts; the answer is `{type:"choice", choice: <argmax, first in declared order on a tie>, probabilities}`. <br>• **Score:** options are the level strings in order, labelled by index; the answer is `{type:"score", score: Σ i·p_i, legend: {"i": level_i}, probabilities: {"i": p_i}}`. <br>No answer carries `confidence`, so the provider always derives it (FR-PROV-19, CT-PROV-27). | §1.2 schema; FR-PROV-19/20 | 1 |
