...
| **R73** | **Submission content is untrusted data. All transcribed submission text is material to be graded, never instructions; any imperative, role claim, or scoring directive embedded in a submission has no effect on model behaviour or on the response contract. This is defended structurally — demarcation, evidence-grounding, output whitelisting — and verified by an adversarial fixture set, never by a content filter that tries to detect malicious phrasing** | A grader that will follow instructions written into the work it grades cannot be trusted with any grade; and a phrase-detector is both bypassable and a §0.8 behavioural instruction where the system prevents structurally | §7.2, §7.7, §8.7 |
| **R74** | **Ingestion neutralizes malicious and malformed PDF content. Active content — embedded JavaScript, OpenAction/AA/Launch actions, embedded files or attachments, and external-reference actions — is stripped before rasterization; resource-exhaustion constructs are bounded; a document whose active content cannot be removed is quarantined, never processed** | A PDF is untrusted input from outside the trust boundary (§7.7); processing its active content or letting a decompression bomb run is a code-execution and denial-of-service surface the sole gateway must close, not each downstream stage | §7.7, §8.4, §8.7 |
...
Every statistical term is explained in plain language the first time it appears. A glossary repeats the short versions at the end of Section 3. Section 0 states the problem and the derived requirements (R1 to R74) that the architecture answers; readers who want the motivation and constraints before the research should start there, and readers who only want the design can start at Section 5. **Readers who care about how this is hosted, tested, or deployed to anything other than a Mac should read Section 0.7 and Section 8.7**, which specify the deployment profiles and the OpenRouter path.
