...
eight contributors: `import aeh.agg, aeh.det, aeh.extract, aeh.ingest, aeh.judge, aeh.orch, aeh.pkg, aeh.synth` —
`import
aeh.pkg` alone is not enough (Tier P's chain is short by `aeh.det`'s migration without it),
`aeh.agg` owns Cohort's last migration (#92's `agg_confidence_columns`, 16), `aeh.judge` the
one before it (#78's `judge_verdict_columns`, 14), `aeh.synth` the
