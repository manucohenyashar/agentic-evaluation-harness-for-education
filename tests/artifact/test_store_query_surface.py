...
KNOWN_EXECUTE_SITES: frozenset[str] = frozenset({
    ...
    # never hand-unioned.)
    # (#373's re-pin: `record_validation_baseline` adds FOUR sites to this module — the
    # version-presence check, the criterion's band ordinals, and the UPDATE/INSERT pair
    # that lands the baseline — and, sitting above the catalog class, moves every site
    # below them. Net +4 against the previous pin; every other entry here is the same
    # statement at a new line. Verified by diffing the walker's (module, source-line-text)
    # multiset against main's, not by reading the numbers. Re-read from the walker,
    # never hand-unioned.)
    "aeh.pkg:2191",
    "aeh.pkg:2330",
    "aeh.pkg:2345",
