...
def test_tc_store_c10_every_missing_precondition_refuses_and_deletes_nothing(tmp_data_dir):
    """`TC-STORE-C10` — the precondition sweep, **exhaustive**: each gate promoted alone
    (the other two missing), then none promoted. Every failing state raises
    `PurgePreconditionError` and leaves the cohort file byte-identical; the naming says
    exactly which gates are unmet. Each state runs in its OWN store directory — Tier D is
    permanent, so a durable file shared across states would accumulate promotions and the
    sweep would be asserting against blurred states."""
    for skip in (-1, 0, 1, 2):
        data_dir = tmp_data_dir / f"state-{skip}"
        ...
        if skip >= 0:
            with sqlite3.connect(store.durable_path()) as raw:
                raw.execute(PROMOTE[skip])
                if skip == 0:
                    raw.execute(
                        "INSERT INTO audit_record (audit_record_id, run_id, recorded_at, "
                        "profile_summary, cohort_id) VALUES ('a-only', 'r', 't', 'p', 's-c10')")
                elif skip == 1:
                    raw.execute(
                        "INSERT INTO label (label_id, run_id, student_ref, criterion_id, "
                        "label_type, band, cohort_id) "
                        "VALUES ('l-only', 'r', 'ref', 'c', 'human', 'b', 's-c10')")
                else:
                    raw.execute(
                        "INSERT INTO criterion_stats (package_version_id, criterion_id, "
                        "backend_profile, panel_build_ref, n, cohort_id) "
                        "VALUES ('pv-only', 'c', 'bp', 'pb-only', 1, 's-c10')")
        path = store.cohort_path("s-c10")
        before = path.read_bytes()
        with pytest.raises(PurgePreconditionError) as refused:
            store.purge_cohort("s-c10")
        assert path.read_bytes() == before, (
            f"TC-STORE-C10 (skip={skip}): the refusal touched the cohort file. The clause's "
            "state assertion is 'nothing deleted', per failing case."
        )
        named = str(refused.value)
        for index, gate in enumerate(GATES):
            if index == skip:
                assert gate not in named, (
                    f"TC-STORE-C10 (skip={skip}): the refusal names {gate!r}, which this "
                    "state promoted. Naming a met gate misdirects the operator."
                )
            else:
                assert gate in named, (
                    f"TC-STORE-C10 (skip={skip}): the refusal does not name the unmet gate "
                    f"{gate!r}. An operator fixing gates one at a time needs the full list."
                )
        store.close()

    # The happy path: everything promoted — the purge deletes, the bytes are gone.
