...
| `aeh.grade:boundary_risk(total, provisional_intervals, boundaries)` | **invented** — FR-GRADE-05's `boundary_at_risk` as a pure seam. `total` is the computed total **including** the provisional criteria's current points; `provisional_intervals` is the **injected interval source** the case requires (test plan §5.14 TC-GRADE-06; the full-band-range assumption is design `TBD` §7.4, CT-GRADE-19): one `(low, high)` offset pair per provisional criterion — where that criterion's eventual points may yet move relative to its current ones, the full declared band range under the conservative assumption. Because each criterion's current points sit inside its own range, `total` always lies inside the achievable span, so `score_low = total + sum(lows)` and `score_high = total + sum(highs)` bracket every achievable outcome; `at_risk` is set when the range has **positive width** and any boundary floor lies inside `[score_low, score_high]` inclusive both ends — landing exactly on a floor resolves to that floor's band (the shipped DDL's floors are INCLUSIVE), which is the case's third scenario, and containment is exactly "the range spans a band edge". A **zero-width** range (`score_low == score_high` — no provisional criteria, or every injected interval collapsed) cannot move the student, so it never spans a band edge and never sets the flag even when the total sits exactly on a floor (#102's degenerate limb, pinned by `test_issue_102_a_degenerate_range_cannot_cross_a_boundary` — the pinned third scenario keeps its positive-width range, so it still fires). Returns `.at_risk`, `.score_low`, `.score_high` (the latter two `None` when not at risk, per CT-GRADE-05). |
