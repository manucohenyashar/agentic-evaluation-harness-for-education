...
| `aeh.grade:boundary_risk(settled_points, provisional_intervals, boundaries)` | **invented** — FR-GRADE-05's `boundary_at_risk` as a pure seam. `provisional_intervals` is the **injected interval source** the case requires (test plan §5.14 TC-GRADE-06; the full-band-range assumption is design `TBD` §7.4, CT-GRADE-19): one `(low, high)` pair of plausible eventual points per provisional criterion. `score_low = settled + sum(lows)`, `score_high = settled + sum(highs)`, and `at_risk` is set when any boundary floor lies inside `[score_low, score_high]` inclusive — inclusive because landing exactly on a floor resolves to that floor's band, which is the case's third scenario. Returns `.at_risk`, `.score_low`, `.score_high` (the latter two `None` when not at risk, per CT-GRADE-05). |
